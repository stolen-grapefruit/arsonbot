...
def setup_motors(control_mode="PWM", use_home_position=True):
    ...
    if use_home_position and INITIAL_POSITION_DEG is not None:
        ...
        print("Sending initial targets:", target_rad)
        ...
        try:
            for dxl_id in motor_group.dynamixel_ids:
                ...
                print(f"Motor {dxl_id} now in mode: {mode}")
        except Exception:
            print("Could not confirm operating mode directly (API may not support it).")

    else:
        print("Skipping home position. Setting control mode directly.")
        motor_group.disable_torque()
        if control_mode == "PWM":
            motor_group.set_mode(DynamixelMode.PWM)
        else:
